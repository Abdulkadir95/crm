<div class="map-record-list-wrapper">
            ...
            <div class="btn-group actions">
                ...
                <div class="btn-group">
                    ...
                    <ul class="dropdown-menu pull-left">
                        ...
                        {{#if this}}
                        <li class="{{#if hidden}}hidden{{/if}}"><a href="javascript:" class="action"
                                                                   data-action="{{name}}">{{#if html}}{{{html}}}{{else}}{{translate label scope=../../../entityType}}{{/if}}</a>
                        </li>
                        {{else}}
                        {{#unless @first}}
                        {{#unless @last}}
            ...
            <div class="sticked-bar hidden">
                <div class="btn-group">
                    ...
                    <ul class="dropdown-menu actions-menu">
                        {{#each massActionList}}
                        {{#if this}}
                        <li><a href="javascript:" data-action="{{./this}}" class='mass-action'>{{translate this category="massActions" scope=../../scope}}</a>
                        </li>
                        {{else}}
                        {{#unless @first}}
                        {{#unless @last}}
            ...
            <table class="table"{{#if tableMinWidth}} style="min-width: {{tableMinWidth}}px;"{{/if}}>
                ...
                <tr>
                    ...
                    <th {{#if width}} width="{{width}}"{{/if}}{{#if align}} style="text-align: {{align}};"{{/if}}>
                        ...
                        {{#if this.isSorted}}{{#unless this.isDesc}}<span class="fas fa-chevron-down fa-sm"></span>{{else}}<span
                                class="fas fa-chevron-up fa-sm"></span>{{/unless}}{{/if}}
                        {{else}}{{label}}{{/if}}
                    </th>
                    {{/each}}
                </tr>
    ...
    <div class="map-record-list-map"></div>
</div>
